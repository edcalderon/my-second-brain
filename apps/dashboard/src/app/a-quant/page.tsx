...
export default function AQuantPortalPage() {
    return (
        <div className="max-w-7xl mx-auto space-y-8 pb-16">
            <section className="border border-slate-200 bg-slate-950 p-6 text-slate-100 sm:p-8 lg:p-10">
                <div className="flex flex-wrap items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.22em]">
                    <span className="border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-cyan-200">/a-quant/</span>
                    <span className="border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-emerald-200">
                        App shell enabled
                    </span>
                    <Link
                        href={dashboardHref("/")}
                        prefetch={false}
                        className="inline-flex items-center gap-1 border border-white/10 bg-white/5 px-3 py-1 text-slate-200 transition hover:border-cyan-300/40 hover:bg-cyan-300/10"
                    >
                        Dashboard home
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <Link
                                href="/portfolio"
                                prefetch={false}
                                className="inline-flex items-center gap-2 border border-cyan-300/40 bg-cyan-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300"
                            >
                            ...
                            <Link
                                href="/market?trading_pair=ETH-USD&interval=1h&limit=120"
                                prefetch={false}
                                className="inline-flex items-center gap-2 border border-white/12 bg-white/5 px-5 py-3 text-sm font-semibold text-slate-100 transition hover:border-white/20 hover:bg-white/10"
                            >
                                Check market feed
                            ...
                            <a
                                ...
                                className="inline-flex items-center gap-2 border border-emerald-400/25 bg-emerald-400/10 px-5 py-3 text-sm font-semibold text-emerald-100 transition hover:border-emerald-300/40 hover:bg-emerald-400/15"
                            >
                                Live API
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                {quickLinks.map((item) => (
                    <Link
                        ...
                        href={item.href}
                        prefetch={false}
                        className="group border border-white/10 bg-white/[0.04] p-5 transition duration-200 hover:border-cyan-300/30 hover:bg-cyan-300/[0.06]"
                    >
                ...
                <div className="border border-white/10 bg-slate-950/70 p-6">
                    ...
                    <div className="mt-6 space-y-3">
                        {checklist.map((item, index) => (
                            <div key={item} className="flex gap-4 border border-white/8 bg-white/[0.03] p-4">
                                <div className="flex h-8 w-8 shrink-0 items-center justify-center border border-cyan-400/20 bg-cyan-400/10 text-sm font-semibold text-cyan-100">
                                    {index + 1}
                                </div>
                    ...
                    <div className="mt-6 space-y-3">
                        {apiLinks.map((item) => (
                            <a
                                ...
                                target="_blank"
                                rel="noreferrer noopener"
                                className="group block border border-white/8 bg-white/[0.03] p-4 transition hover:border-emerald-300/30 hover:bg-emerald-300/[0.08]"
                            >
                                <div className="flex items-center justify-between gap-4">
