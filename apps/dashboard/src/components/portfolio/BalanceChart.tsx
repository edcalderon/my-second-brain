...
import { formatAddress, formatCompactCurrency, formatCurrency, formatNumber, formatPercent, formatTime } from "@/lib/hummingbot-format";
        ...
        <section className="glass-panel rounded-2xl p-6 lg:p-7">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {mainWalletAddress && <InfoChip icon={<TrendingUp className="h-3.5 w-3.5" />} label={formatAddress(mainWalletAddress)} />}
                </div>
            </div>

            <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
                <div className="rounded-xl border border-emerald-200/70 bg-white p-5 shadow-sm dark:border-emerald-900/30 dark:bg-slate-900">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className={`rounded-lg px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] ${trendIsPositive ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300" : "bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300"}`}>
                    ...
                    <div className="mt-6 rounded-xl border border-border bg-white p-4 shadow-sm dark:bg-slate-950">
                        {chart ? (
                            ...
                            <div className="flex h-44 items-center justify-center rounded-lg border border-dashed border-emerald-200/70 bg-white text-sm text-gray-500 dark:border-emerald-900/40 dark:bg-slate-950 dark:text-gray-400">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        ...
                        <StatBadge label="Main USDC" value={mainWalletUsdc === null ? "--" : formatCurrency(mainWalletUsdc, 2)} />
                    </div>
                </div>
...
export function BalanceChartSkeleton() {
    return (
        <section className="glass-panel animate-pulse rounded-2xl p-6 lg:p-7">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="space-y-3">
                    <div className="h-6 w-36 rounded-lg bg-slate-200/80 dark:bg-slate-700/70" />
                    <div className="h-8 w-72 rounded-xl bg-slate-200/80 dark:bg-slate-700/70" />
                    <div className="h-4 w-[min(100%,34rem)] rounded-lg bg-slate-200/70 dark:bg-slate-700/60" />
                </div>
                <div className="flex gap-2">
                    <div className="h-8 w-28 rounded-lg bg-slate-200/70 dark:bg-slate-700/60" />
                    <div className="h-8 w-28 rounded-lg bg-slate-200/70 dark:bg-slate-700/60" />
                </div>
            </div>

            <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
                <div className="rounded-xl border border-border/80 bg-white p-5 dark:bg-slate-900">
                    <div className="h-12 w-64 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                    <div className="mt-5 h-44 rounded-xl bg-gradient-to-b from-slate-200/70 to-slate-100/50 dark:from-slate-700/60 dark:to-slate-800/40" />
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                        <div className="h-14 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                        <div className="h-14 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                        <div className="h-14 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                    </div>
                </div>
                ...
                <div className="space-y-3">
                    <div className="h-24 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                    <div className="h-24 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                    <div className="h-24 rounded-xl bg-slate-200/70 dark:bg-slate-700/60" />
                </div>
            </div>
...
function InfoChip({ icon, label }: { icon: ReactNode; label: string }) {
    return (
        <span className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-600 shadow-sm dark:bg-slate-900 dark:text-gray-300">
            {icon}
            {label}
        </span>
    );
...
function StatBadge({ label, value }: { label: string; value: string }) {
    return (
        <div className="rounded-xl border border-border bg-white px-4 py-3 shadow-sm dark:bg-slate-900">
